...
export async function createOrder(order) {
  const text = `
    INSERT INTO orders (id, status, amount_brl, btc_amount, address, rate_locked, rate_lock_expires_at, created_at)
...
export async function updateOrderStatus(id, status, extra = {}) {
  const { txHash, error } = extra;
  await pool.query(
    `UPDATE orders SET status = $2, tx_hash = COALESCE($3, tx_hash), error = COALESCE($4, error) WHERE id = $1`,
    [id, status, txHash || null, error || null]
  );
  await addEvent(id, `order.${status}`, { txHash, error });
}
